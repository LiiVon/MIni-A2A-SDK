...
namespace a2a
{
    ...
    static size_t stream_callback(void* contents,size_t size,size_t nmemb,void* userp)
    {
        ...
        std::function<void(const std::string&)>* callback = 
            static_cast<std::function<void(const std::string&)>*>(userp);
        (*callback)(std::string(static_cast<char*>(contents), total_size));
        return total_size;
    }
    ...
    struct HttpClient::Impl
    {
        Impl():timeout_(30L)
        {
            ...
            // 非线程安全，hpp中用 unique_ptr 管理生命周期，保证单例
        }
    ...
    HttpResponse HttpClient::get(const std::string& url)
    {
        ...
        // 配置 目的位置、回调调整、内容报存位置、超时设置、自动重定向
        ...
        if(res != CURLE_OK)
        {
            ...
            throw A2AException(std::string("CURL GET request failed: ") + curl_easy_strerror(res), ErrorCode::InternalError);
        }
    ...
    void HttpClient::post_stream(const std::string& url,const std::string& body,
            const std::string& content_type,std::function<void(const std::string&)> callback) 
    {
        CURL* curl = curl_easy_init();
        if (!curl)
        {
